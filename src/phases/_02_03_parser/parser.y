...
program							: GlobalDeclarationList
									{ *program = newProgram(newPosition(1,1), $1); }
								;

GlobalDeclarationList			:
									{ $$ = emptyGlobalDeclarationList(); }
								| NonEmptyGlobalDeclarationList	{ $$ = $1; }
								;

NonEmptyGlobalDeclarationList	: GlobalDeclaration
									{ $$ = newGlobalDeclarationList($1, emptyGlobalDeclarationList()); }
								| GlobalDeclaration NonEmptyGlobalDeclarationList
									{ $$ = newGlobalDeclarationList($1, $2); }
								;
...
TypeDeclaration					: TYPE IDENT EQ TypeExpression SEMIC
									{ $$ = newTypeDeclaration($1.position, $2.val, $4); }
								;
